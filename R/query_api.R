...
parse_xml_and_save <- function(xgeo_url, nb_station, name_station) {
...
read_nve_api <- function(dat = NULL, start_time = NULL, stop_time = NULL, nb_station = NULL, name_station = NULL) {  # , nb_variable = 6046
  ...
  if (is.null(dat) == TRUE) {
    ...
    xgeo_url_corr_modQ <- paste("http://h-web01.nve.no/chartserver/ShowData.aspx?req=getchart&vfmt=xml&ver=1.0&time=",
                      ...
                      ".0.", 6046, ".0],rt=1.0:0,cht=line,mth=mean", sep = "")
    
    # address for ODM air temperature   
    xgeo_url_Temp <- paste("http://h-web01.nve.no/chartserver/ShowData.aspx?req=getchart&vfmt=xml&ver=1.0&time=",
                                start_time,";",stop_time,
                                "&chd=ds=tsr,da=",da,",id=vepshydra[1.",nb_station,
                                ".0.", 6000, ".0],rt=1.0:0,cht=line,mth=mean", sep = "")
    # address for ODM modelled SWE
    xgeo_url_SWE <- paste("http://h-web01.nve.no/chartserver/ShowData.aspx?req=getchart&vfmt=xml&ver=1.0&time=",
                                start_time,";",stop_time,
                                "&chd=ds=tsr,da=",da,",id=vepshydra[1.",nb_station,
    ...
    xmldata <- XML::xmlParse(as.charachter(xgeo_url_corr_modQ))
    tmp <- XML::xmlToDataFrame(XML::getNodeSet(xmldata,"//Point"),colClasses = c("character","double"))
...
#' Parameter codes for ODM:
#' 6000 (mean) air temperature
#' 6004 (mean) Modelled discharge
#' 6005 (mean) Modelled SWE
#' 6014 (mean) Modelled soil water deficit
#' 6046 (mean) Corrected modelled discharge
#' 6001 (sum) Modelled precipitation
#' @param start_day
#' @param stop_day
#'
#' @return
...
read_ODM_api <- function(start_day = Sys.Date() + 1, stop_day = Sys.Date()+10) {
  ...
  station_ref <- read.table("data/HbvFelt147.txt")
  ...
  for (i in seq(along = regine_ref_nb)) {
    single_st_ODM <- read_nve_api(dat = NULL, start_time = start_time, stop_time = stop_time, nb_station = regine_ref_nb[i],
